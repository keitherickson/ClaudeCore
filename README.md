...
| **ffmpeg** | Standard ffmpeg (Gyan full build, via winget). | Per-job **child process** (CLI args) | "Play faster" re-time of an upscaled clip |
...
- **Play faster** — optional re-time of an upscaled clip via **ffmpeg**.
...
```
   ...
   (localhost)           │                                                │
                         │  VideoController ─┐                            │
                         │  UpscaleController ├─► Services (orchestration) │
                         │  AdminController ─┘                            │
                         └───┬───────────────┬───────────────┬───────────┘
                             │ HTTP          │ child process │ child process
                             ▼               ▼               ▼
                   LTX-2.3 server      VideoEffectsApp.exe   ffmpeg.exe
                   127.0.0.1:8765      (Maxine VFX SDK)      (setpts re-time)
                   (Python, LTX        super-resolution      speed change
                    Desktop engine)
                             │
                             ▼
...
2. `POST /Video/Generate`:
   ...
   - The finished file is copied from the server's output path into the app's **output** directory.
...
- SuperRes requires an output height that is an integer **2×/3×/4×** of the source. In the integrated Generate flow, the source height is read with [`VideoProbe`](Services/VideoProbe.cs) — a dependency-free MP4 `tkhd`-box reader — to pick a valid factor.
...
| `VideoSpeedService` | Runs ffmpeg `setpts` re-time. |
...
| `VideoSpeed` | ffmpeg re-time | `FfmpegPath`, `TimeoutMinutes` |
...
```
Controllers/      VideoController, UpscaleController, AdminController, HomeController
...
Views/            Video, Upscale, Admin, Shared (_Layout)
