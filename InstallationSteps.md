...
# 2. start the web app
dotnet run --project C:\ClaudeCore\ClaudeCore\KeithVision.csproj
...
Setup performed:
1. Published a Release build: `dotnet publish KeithVision.csproj -c Release -o C:\ClaudeCore\KeithVision`.
...
2. `C:\ClaudeCore\prompt-venv\Scripts\python -m pip install transformers accelerate fastapi uvicorn` **plus torch for Blackwell** — install torch from the cu128 index *as the primary index*, pinned to the working version: `pip install "torch==2.11.0" --index-url https://download.pytorch.org/whl/cu128` (same wheel as the audio venv). ⚠️ Do **not** use `--extra-index-url`: pip then prefers PyPI's CPU build (`torch==2.12.x+cpu`) and the model silently runs on CPU. Verify with `python -c "import torch; print(torch.version.cuda, torch.cuda.is_available())"` → `12.8 True`.
...
- **2026-06-24** — **Prompt-enhancer environment set up on this machine** (§10): created `C:\ClaudeCore\prompt-venv`, installed transformers/accelerate/fastapi/uvicorn + `torch==2.11.0+cu128`, downloaded `Qwen2.5-7B-Instruct` (~14 GB). Two gotchas: (1) plain `pip install torch` / `--extra-index-url` pulled the **CPU** wheel (`2.12.1+cpu`) — fixed by pinning `torch==2.11.0` with cu128 as the **primary** `--index-url`; now loads on the 4090 (CUDA). (2) PyPI now serves **transformers 5.x**, where `apply_chat_template(return_tensors="pt")` no longer returns a bare tensor, so `model.generate` raised `AttributeError` (500 on `/enhance`). Fixed `tools/prompt_server.py` to use `return_dict=True` + `generate(**enc)` (works on transformers 4.x and 5.x). Validated end to end: `/health` model_loaded:true on cuda, `/enhance` 200 with a proper cinematic rewrite. The **Enhance Prompt** node was hanging with a yellow "running" outline purely because this venv was missing.
...
- **2026-06-20** — **Wan 2.2 14B I2V validated locally (candidate 3rd model for the /Admin switch)**: downloaded the official fp8 set from `Comfy-Org/Wan_2.2_ComfyUI_Repackaged` — `wan2.2_i2v_high_noise_14B_fp8_scaled` + `..._low_noise...` (14 GB each, → `models/diffusion_models`), `umt5_xxl_fp8_e4m3fn_scaled` (6.3 GB, → `text_encoders`), `wan_2.1_vae` (243 MB, → `vae`), and the **lightx2v 4-step LoRAs** high+low (1.2 GB each, → `loras`). Reconstructed the I2V graph from ComfyUI's **bundled template** `video_wan2_2_14B_i2v.json` (dual high/low-noise experts, lightx2v fast path: 4 steps split 2/2, cfg 1.0, `ModelSamplingSD3` shift 5.0, euler/simple, 640×640×81 @ 16fps) into a headless REST script `C:\ComfyUI\run_val_wan22_i2v.py` (mirrors the NVFP4 `run_val_*` pattern: POST /prompt → poll /history → /view). **Validated end-to-end: clean VP9 640×640/81f clip, `Prompt executed in 330.8 s` cold** — but that's dominated by a **one-time ~2:52 high-noise-expert initialization** (Blackwell mixed-precision kernel setup: `Native ops: mxfp8, nvfp4, float8_e4m3fn`); the low-noise expert inits in ~12 s and samples at ~5 s/it, so a **warm gen is ~70–100 s** (matches the template's note; vs LTX-NVFP4 ~20 s — Wan is the quality/i2v model, not the speed one). **No GGUF node needed** — the fp8 files load via the native `UNETLoader` already used for NVFP4, and `run-comfyui.ps1` launched it unchanged. **Next (not done): wire Wan as a 3rd `ILtxVideoBackend`** sharing the ComfyUI process (port 8188) — add `WanVideoBackend` (Key "Wan") + registry entry + the coordinator refinement to skip stopping a backend that shares the target's port (NVFP4↔Wan = same server, no restart; →Wan still stops the LTX Desktop server to free the 5090). Bonus: Wan restores **image-to-video** on a ComfyUI path (NVFP4 backend is t2v-only). No native audio (pair with MMAudio).
- **2026-06-20** — **Wan 2.2 wired into the /Admin switch as the 3rd model (image-to-video, quality)** ✅: added `WanVideoBackend` (Key "Wan") building the validated i2v graph, on a new shared base `ComfyUiBackendBase` (extracted the submit `/prompt` → poll `/history` → download `/view` plumbing from `ComfyUiVideoBackend`, which was refactored onto it with no behavior change; base also adds `/upload/image` so the conditioning frame doesn't depend on a shared filesystem path). New `WanOptions` + `Wan` appsettings section (model/LoRA/encoder/VAE filenames, 4 steps split 2/2, cfg 1.0, shift 5.0, fps 16); registry entry `wan2.2` → "Wan 2.2 (quality, i2v)". **Wan shares the NVFP4 ComfyUI process (port 8188)** — only the graph differs — so the coordinator gained a **same-port skip** (don't stop a backend sharing the target's port) + per-port stop dedupe; switching NVFP4↔Wan never cycles ComfyUI, only →/from the LTX Desktop server starts/stops a process. DI: `WanVideoBackend` HttpClient pointed at the shared ComfyUI BaseUrl. **Verified end-to-end on a dev instance:** 3-model registry lists correctly; NVFP4→Wan kept the same ComfyUI PID (`stopped:[]`); a real render through the app's `POST /Video/Generate` (multipart w/ image) returned `mode:"image-to-video"` and saved a valid **H.264 832×480 / 16fps / 5.06 s** clip (full path: stage image → `/upload/image` → Wan graph → poll → download → H.264 normalize). Build clean (0 warnings). Wan is i2v-only (needs a starting image; clear error otherwise) and has no native audio (pair with MMAudio later). Models live under `C:\ComfyUI\ComfyUI\models` (~28 GB fp8 experts + 6.3 GB umt5 + 2×1.2 GB lightx2v LoRAs + 243 MB VAE); validation script `C:\ComfyUI\run_val_wan22_i2v.py`.
- **2026-06-21** — **AI video upscaling via ComfyUI (arbitrary target resolution) — 2nd upscale engine alongside Maxine** ✅: Maxine SuperRes is locked to integer 2×/3×/4× (the SDK rejects other ratios), so added an AI engine that upscales to **any** resolution. New `ComfyUiUpscaleService` + `ComfyUiUpscaleOptions` (`ComfyUiUpscale` config) running the validated graph on the shared ComfyUI server: `LoadVideo → GetVideoComponents → UpscaleModelLoader + ImageUpscaleWithModel` (Real-ESRGAN x4, downloaded to `models/upscale_models`) `→ ImageScale` (exact target W×H) `→ CreateVideo` (re-attach source fps+audio) `→ SaveVideo`; validated headlessly via `C:\ComfyUI\run_val_upscale.py`. The service **auto-starts ComfyUI** if down (reuses `ComfyUiServerControl`), and because ComfyUI decodes any codec + preserves audio/fps there's **no H.264 pre-transcode or audio re-mux** (unlike Maxine). New `UpscaleController.RunAi` takes a **target height**; `VideoProbe` gained `TryGetDimensions` so the controller computes an aspect-preserved even width. `/Upscale` page got an **engine toggle** (Maxine factor / AI target-resolution) + a height picker (720p/1080p/1440p/4K); health badge reflects the selected engine. Maxine code kept fully intact. **Verified end-to-end through the app** at 1080p (1920×1080), 1440p (2496×1440), and **4K (3744×2160)** — aspect preserved, audio/fps intact, ComfyUI auto-started from cold. Build clean. Note: AI upscaling is frame-by-frame (~1 s/frame at x4) so long clips are slow — Maxine stays the fast option; for 4K from a sub-1080p source the ESRGAN x4 output is resized *up* past 4×.
- **2026-06-21** — **Project renamed ClaudeCore → KeithVision**: the .NET project, assembly, and all namespaces (`ClaudeCore.* → KeithVision.*`, 54 files) renamed to match the user-facing app name. `ClaudeCore.csproj → KeithVision.csproj` (so the exe is now **`KeithVision.exe`**); launch/publish/install scripts updated (`start-keithvision.ps1` → `KeithVision.exe`, `publish-keithvision.ps1` → `KeithVision.csproj` + `Stop-Process KeithVision`). **Deliberately unchanged:** the repo folder `C:\ClaudeCore\ClaudeCore` (moving it would break the hardcoded tool-path defaults and collide with the publish dir `C:\ClaudeCore\KeithVision`) and the GitHub repo name (`keitherickson/ClaudeCore`). First deploy after the rename needed a one-time migration: stop the old `ClaudeCore.exe`, clear stale `ClaudeCore.*` from the publish dir, then publish `KeithVision.exe`. Build clean (0 warnings); renamed app verified booting (binds, DI resolves, endpoints serve).
- **2026-06-21** — **Split into two projects: `KeithVision` (UI) + `KeithVision.Core` (services)**: extracted the service layer into a `KeithVision.Core` class library (`Services/` 31 files + `Models/` 3 files moved in, namespaces unchanged), referenced by the `KeithVision` web project. All DI registration moved out of `Program.cs` into `KeithVision.Core/ServiceCollectionExtensions.cs` → `services.AddKeithVisionServices(configuration)` (one call; `Program.cs` is now just MVC + that call + the pipeline). This sets up service reuse for a future MCP host. Core uses `<FrameworkReference Include="Microsoft.AspNetCore.App" />` (so `IFormFile`/`ILogger`/`IOptions`/`BackgroundService` resolve with the exact framework the web host uses — no NuGet version juggling) plus the global usings the Web SDK had provided implicitly (`Microsoft.AspNetCore.Http`, `Microsoft.Extensions.Logging`, etc.). Web project keeps its name/exe (`KeithVision.exe`) so the launch/publish/scheduled-task infra is untouched; it excludes the nested `KeithVision.Core\**` from its globs and adds a `ProjectReference`. New `KeithVision.slnx` (the .NET 10 default solution format) ties both. **Only web coupling left in Core is `IFormFile`** in 5 upload-staging methods — swapping those to `Stream` would let Core drop the FrameworkReference (deferred). Verified: solution builds clean (0 warnings) → `KeithVision.Core.dll` + `KeithVision.dll`; app boots; every controller resolves its Core services over real HTTP (`/Admin/Models`, `/Upscale/Health`, `/Video/Health`); published live (Core.dll deployed alongside the exe).
- **2026-06-21** — **KeithUI hosted locally at `http://www.keithui.com`** ✅: gave the new node-graph studio (the `new-ui` branch app) the same local-hostname treatment as KeithVision, running side-by-side on the same box. Since KeithVision already owns `127.0.0.1:80`, KeithUI binds its **own loopback IP `127.0.0.2:80`** (`KeithUI/appsettings.json` → `Kestrel:Web`; kept `127.0.0.1:5099` as `WebAlt` for the dev preview) — two apps on port 80, different addresses, no conflict (verified `127.0.0.2` is a usable loopback on Windows; both listeners coexist). hosts file got `127.0.0.2  www.keithui.com keithui.com`. Published to **`C:\ClaudeCore\KeithUI`** (`dotnet publish -c Release`, mirrors `C:\ClaudeCore\KeithVision`). New `tools/start-keithui.ps1` launches the published `KeithUI.exe` hidden if its endpoint isn't up — guard is **address-specific** (`Get-NetTCPConnection -LocalAddress 127.0.0.2 -LocalPort 80`) because a bare port-80 check always sees KeithVision's `127.0.0.1:80` and would never start KeithUI. New **"KeithUI Startup"** logon scheduled task (mirrors "KeithVision Startup": user `DAPAD\keith`, logon trigger, RunLevel **Limited** — so binding :80 needs no admin, same as KeithVision). Elevated, idempotent installer `tools/install-keithui-host.ps1` does the hosts edit + task registration (one UAC prompt). KeithUI **shares KeithVision's backends** (LTX :8765, ComfyUI :8188, audio :8770) and the GPU — no separate queue, so concurrent jobs from both UIs contend. **Verified end-to-end:** `www.keithui.com` resolves to 127.0.0.2, `GET http://www.keithui.com/` → 200 serving KeithUI, KeithVision still 200 (no regression), task Ready. **Dev gotcha:** the published instance now also holds `127.0.0.1:5099`, so stop it before running the dev preview (which expects 5099). Repo changes on branch `new-ui`. *(superseded — see next entry: dev and hosted now coexist.)*
- **2026-06-21** — **KeithUI dev/hosted binding decoupled (no more port-5099 fight)** ✅: the hosted instance had been binding `127.0.0.1:5099` too (the `WebAlt` endpoint), squatting the dev-preview port so every dev run had to kill the live site. Removed `Kestrel:Endpoints` from `KeithUI/appsettings.json` entirely and drive each context's URL explicitly: **hosted** = `ASPNETCORE_URLS=http://127.0.0.2:80` set in `start-keithui.ps1` (which no longer guards/binds 5099); **dev preview** = `applicationUrl=http://127.0.0.1:5099` in the launchSettings `http` profile. A shared `Kestrel:Endpoints` block was the trap — it pins both contexts to the same address *and* (by config precedence) overrides `--urls`/`ASPNETCORE_URLS`, so per-context env/args couldn't take effect until it was gone. **Key footgun found & fixed:** the dev preview must stay in the **Development** environment — a non-published `dotnet run` in **Production** *disables Static Web Assets* and serves `studio.js`/`litegraph.js` as **empty 0-byte 200s** (page shell renders, `LiteGraph` undefined, blank canvas). So `.claude/launch.json` keeps the plain `--no-build` (Development profile applies); do **not** add `--no-launch-profile`/`--urls` (flips it to Production). Verified both up simultaneously: hosted `www.keithui.com` → 200 on `127.0.0.2:80`, dev preview serving full 1.07 MB assets on `127.0.0.1:5099`, KeithVision still 200 on `127.0.0.1:80`. Re-published; `new-ui` commit `37b9fd7`.
- **2026-06-24** — **KeithUI studio feature set** ✅ (CI-verified compile): a batch of node-graph additions, merged via PR #1. **Add Audio** node (`Sound/add_audio`) muxes an audio track onto a finished clip (`VideoSpeedService.MuxAudioAsync` — video stream copied, audio → AAC, capped to the video's duration). **Named layouts** persisted server-side (`LayoutStore`, one JSON per layout under `App_Data/studio-layouts`) with a toolbar dropdown showing save-times and an overwrite warning that mirrors the server-side slug. **Run cancellation**: a shared `RunRegistry` lets a separate request cancel an in-flight run — a Stop button in the studio (AbortController + `/Admin/CancelRun`) and a Cancel button per active run in Admin; the executor already checks the token cooperatively. **Run Group** node (`Groups/run_group`) runs a saved layout as a self-contained sub-graph (executor refactored into `ExecuteGraphAsync`, reused at depth+1 with a depth cap + group-name cycle guard) and emits its result downstream. **Studio admin dashboard** (`/Admin`, KeithUI-owned, separate from the v1 `/Admin`): backends + model switch, GPU + per-process VRAM, diagnostics, storage, a recent-outputs gallery (inline preview/delete), and live generation progress. **CI**: `.github/workflows/build.yml` builds the three net10.0 projects on every push/PR (the .NET 10 SDK download hosts are blocked inside the Claude Code remote env, so GitHub runners provide the compile check). **Docs/deploy**: `REMOTE-ACCESS.md` (Tailscale+RDP / Parsec) and `tools/publish-keithui.ps1` (the missing one-command KeithUI publisher, mirrors `publish-keithvision.ps1`).
- **2026-06-24** — **Two-GPU targeting: video → 5090, audio → 4090, by card name** ✅ (CI-verified compile): with the 4090 now installed, made the GPU split robust. Previously index-based (`CUDA_VISIBLE_DEVICES=0/1`), which is fragile — under `CUDA_DEVICE_ORDER=PCI_BUS_ID` index 0 is whichever card is on the lower PCIe bus, so a re-seat/BIOS reorder could silently move video onto the 4090. New `tools/gpu-common.ps1` `Resolve-GpuIndex` maps a model name (e.g. "RTX 5090") to its CUDA ordinal via nvidia-smi (slot-order-proof); the launch scripts (`run-ltx`, `restart-ltx`, `run-comfyui`, `run-audio`, `start-keithvision`) resolve by name (`-GpuName`, video → RTX 5090 / audio → RTX 4090) with the `-Gpu` index as fallback. `MaxineUpscaleService` pins VideoEffectsApp to the 5090 by exporting `CUDA_VISIBLE_DEVICES` on the child process (`Maxine:GpuName`). And now that audio has a dedicated 4090, `audio_server.py` loads the model **resident** on the GPU instead of parking it on CPU between generations (the parking was a single-shared-GPU workaround) — saves the ~2 s host↔device copy per call.
- **2026-06-24** — **Self-hosted prompt enhancer — local LLM on the 4090** ✅ (CI-verified compile): puts the second card to work. New local Python server `tools/prompt_server.py` (FastAPI + transformers, default `Qwen/Qwen2.5-7B-Instruct`, resident on the 4090) on `127.0.0.1:8771`, launched by `tools/run-prompt-server.ps1` (pinned to the 4090 by name); `POST /enhance {text,style,model}` → `{prompt}`. Core stack mirrors the audio one: `LocalLlmClient` (typed HttpClient) → `PromptEnhanceService` (auto-starts the server on first use; falls back to the raw text on failure) → `PromptServerControl` (start/stop), registered in `ServiceCollectionExtensions`. New **Enhance Prompt** node (`Prompts/enhance`, idea + style + model dropdown) with a `TEXT` output that feeds a Generate/Extend Video `prompt` input (the executor prefers a wired prompt over the box). The node's model dropdown swaps the resident model server-side on demand (frees the old one first). Admin shows the prompt server with Start/Stop; new `LocalLlm` config section; install steps in §10. One-time setup on the box: a `prompt-venv` (transformers + Blackwell torch); model downloads (~15 GB) on first use.
- **2026-06-27** — **New-machine / new-profile rebuild** (migrated `C:\` drive, fresh Windows profile `DAPAD\keith`). The data on `C:\` survived (`C:\ComfyUI`, `C:\ClaudeCore`) but everything user-scoped was gone (toolchain, per-user AppData incl. LTX Desktop, the audio/prompt venvs, scheduled tasks). Done: **(1) Toolchain** — winget-installed .NET 10 SDK (10.0.301), GitHub CLI (2.95.0), ffmpeg (Gyan 8.1.2), Python 3.11.9; set git identity. **(2) Build fixes** — `appsettings.Development.json` had stray garbage before the opening brace (committed, crashed `dotnet run` in Development) — fixed; `VideoSpeed:FfmpegPath` repointed to the installed ffmpeg 8.1.2 build. Solution builds 0/0; app verified serving `/Video` `/Upscale` `/Admin` (HTTP 200). **(3) ComfyUI** — `install-comfyui.ps1` re-ran clean (cu130 nightly torch + deps already present); fixed git "dubious ownership" on the old-SID `C:\ComfyUI` repos (`safe.directory` for ComfyUI `5ef0092` + LTX node `4f45fd6`, kornia patch intact); all ~112 GB of weights verified present (`download-comfyui-models.ps1` → every file skipped); launched the server, bound `:8188`, confirmed pinned to the **RTX 5090** (cuda:0, 31.8 GB). **(4) Maxine** — VFX SDK re-cloned to `C:\ClaudeCore\maxine-vfx` (prebuilt exe present). **(5) Audio + prompt venvs** — recreated `C:\ClaudeCore\audio-venv` and `prompt-venv` from their pinned requirements (torch 2.11.0+cu128, CUDA available; `StableAudioPipeline`/`transformers` import OK). **Still needs the user (EULA / gated, can't be automated):** install **LTX Desktop** (ltx.io, run once to fetch ~70 GB weights → the default `bf16-2.3` model + `tools/run-ltx-server.ps1`); install the **Maxine Video Effects SDK redistributable** (then robocopy models to `C:\ClaudeCore\maxine-models`); accept the two **Stable Audio Open 1.0** HF licenses + `hf auth login` for the ~9.7 GB weights. ComfyUI video (NVFP4 + Wan 2.2) and AI upscale are fully working now; the prompt enhancer's Qwen model auto-downloads on first use. (Operational extras — Release publish to `C:\ClaudeCore\KeithVision`, the logon scheduled tasks, and the `keithvision.com`/`keithui.com` hosts entries — were also profile-scoped and are not yet re-done.)
